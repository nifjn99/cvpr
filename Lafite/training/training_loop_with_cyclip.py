...
from pkgs_cyclip.openai.clip import load as load_model
...
#                'A man has blue hair and no beard']
        ...
        with torch.no_grad():
            clip_model, clip_processor = load_model(name = 'RN50', pretrained = False)  # Load CLIP model here
            checkpoint = 'best_cyclip.pt'
            ...
            for step in range(len(text)):
                tokenized_text = clip_processor.process_text(text[step])
                tokenized_text = tokenized_text['input_ids'].to(device)
                txt_fts = clip_model.get_text_features(tokenized_text).view(1, -1).repeat(labels.shape[0], 1)
    ...
    while True:
        ...
        if batch_num % 2000 == 0: 
            ...
            if True:
                ...
                snapshot_pkl = os.path.join(run_dir, f'network-snapshot-{batch_num:06d}.pkl')
                if rank == 0:
