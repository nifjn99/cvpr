...
#         print(torch.cosine_similarity(img_fts, txt_fts, dim=-1))
        ...
        shift = torch.load('training/gap_img_to_txt64.pt')
        shift.requires_grad = False
        #shift = shift.expand(64, 512)
        shift = shift.to(img_fts.device)
...
#                 img_fts_ = self.mapper(img_fts_real_full_) + img_fts_real_full_
            ...
            # learn some images with noise based on mixing_prob
            
            #num_noise = int(img_fts_.shape[0]) * mixing_prob
            #idx_noise = torch.cat((torch.ones(int(num_noise), 1), torch.zeros(img_fts_.shape[0] - int(num_noise), 1)), dim=0).to(img_fts_.device)
            
            #txt_fts_ = torch.where (idx_noise == 1, random_noise, img_fts_)
    ...
    ### change noise level        
        def accumulate_gradients_noise09(self, phase, real_img, real_c, gen_z, gen_c, sync, gain, img_fts, txt_fts, lam, temp, gather, d_use_fts, itd, itc, iid, iic, mixing_prob=0.):
            assert phase in ['Gmain', 'Greg', 'Gboth', 'Dmain', 'Dreg', 'Dboth']
            do_Gmain = (phase in ['Gmain', 'Gboth'])
            do_Dmain = (phase in ['Dmain', 'Dboth'])
            do_Gpl   = (phase in ['Greg', 'Gboth']) and (self.pl_weight != 0)
    ...
    #                 img_fts_ = self.mapper(img_fts_real_full_) + img_fts_real_full_
        ...
        def accumulate_gradients_noise06(self, phase, real_img, real_c, gen_z, gen_c, sync, gain, img_fts, txt_fts, lam, temp, gather, d_use_fts, itd, itc, iid, iic, mixing_prob=0.):
            assert phase in ['Gmain', 'Greg', 'Gboth', 'Dmain', 'Dreg', 'Dboth']
            do_Gmain = (phase in ['Gmain', 'Gboth'])
            do_Dmain = (phase in ['Dmain', 'Dboth'])
            do_Gpl   = (phase in ['Greg', 'Gboth']) and (self.pl_weight != 0)
