...
DATA_NUM_PER_DATASET = 1000000
IMAGE_SNAPSHOT_TICKS = 10
MAX_SIZE = 1000000
RESOLUTION = 256
PATHS_DATASET = ['./../j-nakao/data/frc_improved_split0.zip',
                 './../j-nakao/data/frc_improved_split1.zip',
                 './../j-nakao/data/frc_improved_split2.zip',
                 './../j-nakao/data/frc_improved_split3.zip',
                 './../j-nakao/data/frc_improved_split4.zip',
                 './../j-nakao/data/frc_improved_split5.zip',
                 './../j-nakao/data/frc_improved_split6.zip',
                 './../j-nakao/data/frc_improved_split7.zip',
                 './../j-nakao/data/frc_improved_split8.zip',
                 './../j-nakao/data/frc_improved_split9.zip']

def make_training_set_kwargs(idx: int, path: str):
...
def make_training_set_kwargs_list(paths: list):
    training_set_kwargs_list = []
    for idx, path in enumerate(paths):
        training_set_kwargs = make_training_set_kwargs(idx, path)
        training_set_kwargs_list.append(training_set_kwargs)
    return training_set_kwargs_list
...
#     M_kwargs                = {},
    ...
    image_snapshot_ticks    = IMAGE_SNAPSHOT_TICKS,       # How often to save image snapshots? None = disable.
...
#                 previous_w_list = current_w_list
    ...
    cnt_trained_data = DATA_NUM_PER_DATASET + 1
    idx_dataset = 0
    training_set_kwargs_list = make_training_set_kwargs_list(PATHS_DATASET)
    ...
    while True:
        if cnt_trained_data >= DATA_NUM_PER_DATASET:
            print(f"Loading dataset from {training_set_kwargs_list[idx_dataset]['path']}")
            idx_dataset += 1
            if idx_dataset >= len(training_set_kwargs_list):
                idx_dataset = 0
            cnt_trained_data = 0
            training_set = dnnlib.util.construct_class_by_name(**training_set_kwargs_list[0])
            training_set_sampler = misc.InfiniteSampler(dataset=training_set, rank=rank, num_replicas=num_gpus, seed=random_seed)
            training_set_iterator = iter(torch.utils.data.DataLoader(dataset=training_set, sampler=training_set_sampler, batch_size=batch_size//num_gpus, **data_loader_kwargs))
        # Fetch training data.
