...
@click.option('--width', help='Output width', type=int)
@click.option('--height', help='Output height', type=int)
def convert_dataset_with_cyclip(
    ctx: click.Context,
    source: str,
    dest: str,
    max_images: Optional[int],
    ...
    device = 'cuda'
    PIL.Image.init() # type: ignore
    model, processor = load_model(name = 'RN50', pretrained = False) # Load CyCLIP model here
    ...
    #             error(f'Image {archive_fname} attributes must be equal across all images of the dataset.  Got:\n' + '\n'.join(err))
                with torch.no_grad():
                    # Save the image as an uncompressed PNG.
                    img = PIL.Image.fromarray(img, { 1: 'L', 3: 'RGB' }[channels])
                    feature = torch.zeros(1, 1024).cuda()
                    ...
                    for _ in range(cut_num_): # random crop and resize to get the average feature of image
                        ...
                        normed_img = clip_preprocess()(reshaped_img).cuda()
                        with torch.no_grad():
                            feature += model.get_image_features(normed_img)
                    feature  = feature/cut_num_
                    ...
                    for text_line in text:
                        if text_line != '' and not text_line.isspace():
                            try:
                                ...
                                text_feature = 0.
                                for te in tokenized_text:
                                    text_feature += model.get_text_features(te)/len(tokenized_text)
                                text_feature_list.append(text_feature.view(-1).cpu().numpy().tolist())
                                print('text too long')
...
if __name__ == "__main__":
    #convert_dataset_with_cyclip() # pylint: disable=no-value-for-parameter
    convert_dataset()